/***********************************************************
 ...
 binTree.h file to successfully compile and run.
 ************************************************************/
...
template <typename T>     // Start of the binTree template class.
class binTree
{
...
protected:     // Declaring the protected data type (and some additional methods) for the binTree template class.
    ...
    // Implementing the RNG here so that we can randomly select the left subtree or the right subtree (in the private version of the insert() method).
...
/* This public version of the size() method just calls the private version of the size() method by passing in
   the actual root of the entire binary tree (called root) as an argument. Then, this method returns the size
   of the binary tree which we got from calling the private version of the size() method (the size of the binary
   tree is the sum of all nodes in the entire binary tree). */
template <typename T>
...
/* This private version of the height() method checks to see if the heightRoot is equal to nullptr, if it
   ...
   original value to count the root, this method returns the value of the height of the entire binary tree. */
template <typename T>
unsigned binTree<T>::height(Node <T> *heightRoot) const
{
    ...
    // Creating a local variable of type unsigned integer called leftTreeHeight which holds the height of the left subtree of the binary tree.
    unsigned leftTreeHeight;
    // Creating a local variable of type unsigned integer called rightTreeHeight which holds the height of the right subtree of the binary tree.
    unsigned rightTreeHeight;
    
    leftTreeHeight = height(heightRoot->left);
    rightTreeHeight = height(heightRoot->right);
    
    treeHeight = max(leftTreeHeight, rightTreeHeight) + 1;
...
/* This private version of the size() method checks to see if the sizeRoot is equal to nullptr, if it
   is, then this method returns 0. Otherwise, this size() method gets the size of the left subtree of
   the binary tree and the size of the right subtree of the binary tree. Finally, this method gets the
   sum of the leftTreeSize, the rightTreeSize, and the root (which is just plus one), and returns that
   value (called treeSize). */
template <typename T>
unsigned binTree<T>::size(Node <T> *sizeRoot) const
{
    
    // Creating a local variable of type unsigned integer called treeSize which holds the size of the binary tree (entire number of nodes in the tree).
    unsigned treeSize;
    ...
    // Creating a local variable of type unsigned integer called leftTreeSize which holds the size of the left subtree of the binary tree.
    unsigned leftTreeSize;
    // Creating a local variable of type unsigned integer called rightTreeSize which holds the size of the right subtree of the binary tree.
    unsigned rightTreeSize;
    
    leftTreeSize = size(sizeRoot->left);
    rightTreeSize = size(sizeRoot->right);
